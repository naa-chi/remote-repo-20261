...
public class EngineController {
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved the engine record", content = @Content(schema = @Schema(implementation = EngineResponseDTO.class))),
        @ApiResponse(responseCode = "404", description = "Engine not found")
    })
    @GetMapping("/{id}")
    public ResponseEntity<EngineResponseDTO> getEngineById(
            @Parameter(description = "Primary key database ID", example = "1") @PathVariable Long id) {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "No engines found for the specified manufacturer")
    })
    @GetMapping("/manufacturer/{code}")
    public ResponseEntity<CollectionModel<EngineResponseDTO>> getEnginesByManufacturerCode(
            @Parameter(description = "Manufacturer code", example = "M123") @PathVariable String code) {
    ...
    public ResponseEntity<CollectionModel<EngineResponseDTO>> getEnginesByEngineCode(
            @Parameter(description = "Engine model code", example = "V8-Turbo") @PathVariable String engineCode) {
    ...
    public ResponseEntity<CollectionModel<EngineResponseDTO>> getEnginesByHorsepower(
            @Parameter(description = "Horsepower value", example = "4400.0") @PathVariable Float horsepower) {
    ...
    public ResponseEntity<CollectionModel<EngineResponseDTO>> getEnginesByPrice(
            @Parameter(description = "Price value", example = "150000.0") @PathVariable Float price) {
    ...
    public ResponseEntity<CollectionModel<EngineResponseDTO>> getEnginesByWeight(
            @Parameter(description = "Weight value", example = "120.5") @PathVariable Float weight) {
    ...
    @PostMapping("/create")
    public ResponseEntity<EngineResponseDTO> createEngine(@Valid @RequestBody EngineRequestDTO engineDTO) {
    ...
    public ResponseEntity<EngineResponseDTO> updateEngine(
            @Parameter(description = "Engine ID", example = "1") @PathVariable Long id,
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "System processing finalized"),
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteEngine(@Parameter(description = "Engine ID", example = "1") @PathVariable Long id) {
